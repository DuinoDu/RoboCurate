"""Build and verify a v3 handoff with frozen experiments and explicit runtime files."""
from datetime import datetime
import hashlib,importlib.util,json,stat,xml.etree.ElementTree as ET,zipfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2];STUDY=ROOT/'research/stage_v3_motion_20260917';APP=ROOT/'robot-data-studio'
PARENT=ROOT/'HoloCurate-多模态时序升级-20260917-163825.zip'
PARENT_SHA='9d38194bda361aae2475c5a2d4b5619c1d8bf8384968069b37e064e710a25ea4'
...
def main():
    ...
    original=json.loads((STUDY/'installed_model.before-review-guard.json').read_text())
    path=APP/'workspace/warp/models'/original['checkpoint'];assert sha(path.read_bytes())==original['checkpoint_sha256'];add(path)
    for folder in [ROOT/'research/stage_v3_20260917',STUDY]:
    ...
    evidence=APP/'workspace/evidence/stages-v3'
    ...
    assert counts==dict(tests=109,failures=0,errors=0,skipped=0)
    ...
    now=datetime.now().astimezone();target=ROOT/('HoloCurate-运动感知模型升级-'+now.strftime('%Y%m%d-%H%M%S')+'.zip')
    assert not target.exists()
    manifest=dict(schema_version=4,built_at=now.isoformat(),parent_archive_sha256=PARENT_SHA,
        scope='Motion-aware task stage research model; iterated same-session CV gain, unchanged historical stage accuracy; no final-success or policy-improvement claim',
        models=[dict(id=p['id'],sha256=p['checkpoint_sha256']) for p in chosen],
        validation=dict(python_regression=counts,node_test_files=len(list((APP/'tests').glob('*.test.mjs'))),browser_checks=ui,runtime=verify),
